      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl p-8 w-full max-w-md transform transition-all">
        ...
        <div className="space-y-5">
          ...
          <div className="flex justify-end space-x-3">
            <button
              onClick={onClose}
              className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition"
            >
              Cancel
            </button>
            <button
              onClick={handleSubmit}
              className="px-4 py-2 bg-[#992787] dark:bg-purple-600 text-white rounded-lg hover:bg-[#7a1f6e] dark:hover:bg-purple-700 transition"
            >
              Save Changes
            </button>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 dark:bg-gray-900">
        ...
        <div className="pt-20 px-8 pb-8">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-[#992787] dark:text-purple-400 font-medium mt-1">{profile.role}</p>
              <p className="text-gray-600 dark:text-gray-300 mt-3 max-w-2xl">{profile.bio}</p>
            </div>
            <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        {['projects', 'connections', 'completion'].map((stat, index) => (
          <div key={index} className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm hover:shadow-md transition">
            <h3 className="text-lg font-semibold text-gray-700 dark:text-gray-300">
              {stat === 'projects' && 'Events Completed'}
              {stat === 'connections' && 'Employees'}
              {stat === 'completion' && 'Profile Strength'}
            </h3>
            {stat === 'completion' ? (
              <div className="mt-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mt-8">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-100 mb-4">Social Connections</h2>
          <div className="space-y-3">
            <div className="flex items-center gap-3">
              <svg className="w-5 h-5 text-[#1DA1F2]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z" />
              </svg>
              <span className="text-gray-600 dark:text-gray-300">{profile.social.twitter}</span>
            </div>
            <div className="flex items-center gap-3">
              <svg className="w-5 h-5 text-[#0077b5]" fill="currentColor" viewBox="0 0 24 24">
                <path d="M4.98 3.5c0 1.381-1.11 2.5-2.48 2.5s-2.48-1.119-2.48-2.5c0-1.38 1.11-2.5 2.48-2.5s2.48 1.12 2.48 2.5zm.02 4.5h-5v16h5v-16zm7.982 0h-4.968v16h4.969v-8.399c0-4.67 6.029-5.052 6.029 0v8.399h4.988v-10.131c0-7.88-8.922-7.593-11.018-3.714v-2.155z" />
              </svg>
              <span className="text-gray-600 dark:text-gray-300">{profile.social.linkedin}</span>
            </div>
          </div>
        </div>
      </div>
